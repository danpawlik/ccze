...
static char *ccze_color_to_html (int color);
...
ccze_addstr (int col, const char *str)
{
  if (ccze_config.html)
    {
      if (str)
	printf ("%s%s</font>", ccze_color_to_html (col), str);
    }
  else
    {
...
static char *
ccze_color_to_name_html (int color, int realcolor)
{
...
static char *
ccze_color_to_html (int color)
{
  int my_color = ccze_color_strip_attrib (ccze_color (color));
  int i = (my_color >> 8) % 8;
  int j = (my_color >> 8) / 8;
  char *str;

  if (my_color < COLOR_PAIR (8))
    asprintf (&str, "<font color=\"%s\">",
	      ccze_color_to_name_html (my_color, color));
  else
    asprintf (&str, "<font color=\"%s\" style=\"text-background: %s\">",
	      ccze_color_to_name_html (COLOR_PAIR (i), color),
	      ccze_color_to_name_html (COLOR_PAIR (j), color));
...
main (int argc, char **argv)
{
  ...
  if (!ccze_config.html)
	...
	("<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01//Transitional//EN\">\n"
	 ...
	 "<title>Log colorisation generated by %s</title>\n"
